...
def create_demo_run_bundle(root: Path) -> str:
    ...
    # Fail-closed demo: must be UNSAT verified by all three checkers
    if res.result != "UNSAT":
    ...
    manifest: dict[str, Any] = {
        ...
        "candidate": {
            "executor": "cadical",
            "genome": {"seed": 0, "notes": "three-checker UNSAT verification"},
            "commandline": res.commandline,
            "proof_policy": {
                "unsat_requires_three_checkers": True,
                "checkers": ["drat-trim", "lrat-trim", "cake_lpr"],
                "cake_lpr_note": "HOL4-formally-verified via CakeML (tanyongkiam/cake_lpr)",
            },
        },
        "outputs": {
            "result": res.result,
            ...
            "cake_lpr_check_path": "proofcheck/cake_lpr.json",
        },
        "hashes": {},
    }
    ...
    for rel in [
        res.drat_relpath,
        res.lrat_relpath,
        "proofcheck/drat-trim.json",
        "proofcheck/lrat-trim.json",
        "proofcheck/cake_lpr.json",
    ]:
        if rel:
            p = out_dir / rel
            if p.exists():
